...
export default function ReviewQueue() {
  ...
  // Hardcoded rules for demo purposes (would normally come from active persona)
  const personaIdentity = "You are a warm, supportive, and knowledgeable financial educator.";
  const personaRules = [
  ...
  const loadQueue = async () => {
    setIsLoading(true);
    try {
      const items = await QueueAPI.getPendingItems();
      setQueue(items);
      if (items.length > 0) {
  ...
  const handleResolve = async (action: 'approve' | 'reject' | 'edit') => {
    ...
    try {
      await QueueAPI.resolveItem(currentItem.id, action, draftedText);
      // Move to next item
                ...
                <div className="flex justify-between items-start mb-6">
                  <div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs bg-purple-500/10 text-purple-400 px-2 py-0.5 rounded border border-purple-500/20">Gemini 3 Flash</span>
                    </div>
                  </div>
                  ...
                  {isDrafting ? (
                    <div className="flex-1 bg-[#0B0F1A] border border-[#2D3748] rounded-xl flex items-center justify-center p-4">
                      <div className="flex items-center gap-3 text-gray-400">
                        ...
                        <span className="text-sm">Gemini is drafting a response...</span>
                      </div>
                    </div>
                  ) : (
